...
FRESULT cFile::lseek (DWORD ofs) {
  ...
  if (cltbl) {
      ...
      if (ofs) {
        ...
        if (fptr % SECTOR_SIZE && dsc != dsect) {
          ...
          if (flag & FA__DIRTY) {
            ...
            if (diskWrite (fs->drv, buf.d8, dsect, 1) != RES_OK)
              ABORT (fs, FR_DISK_ERR);
            flag &= ~FA__DIRTY;
            }
          if (diskRead (fs->drv, buf.d8, dsc, 1) != RES_OK) /* Load current sector */
            ABORT(fs, FR_DISK_ERR);
          dsect = dsc;
          }
        }
    ...
    if (fptr % SECTOR_SIZE && nsect != dsect) {
      ...
      if (diskRead (fs->drv, buf.d8, nsect, 1) != RES_OK)
        /* Fill sector cache */
        ABORT(fs, FR_DISK_ERR);
      dsect = nsect;
      }
  ...
  for ( ;  btr; /* Repeat until all data read */ rbuff += rcnt, fptr += rcnt, *br += rcnt, btr -= rcnt) {
    if ((fptr % SECTOR_SIZE) == 0) {
      ...
      if (dsect != sect) {
        ...
        if (diskRead (fs->drv, buf.d8, sect, 1) != RES_OK)  /* Fill sector cache */
          ABORT(fs, FR_DISK_ERR);
        }
      dsect = sect;
      }
  ...
  for ( ;  btw; wbuff += wcnt, fptr += wcnt, *bw += wcnt, btw -= wcnt) {
    ...
    if ((fptr % SECTOR_SIZE) == 0) {
      ...
      if (dsect != sect) {
        ...
        if (fptr < fsize && diskRead (fs->drv, buf.d8, sect, 1) != RES_OK)
          ABORT(fs, FR_DISK_ERR);
        }
      dsect = sect;
      }
