...
static DWORD get_fat (FATFS* fs, DWORD clst) {
  ...
  if (clst < 2 || clst >= fs->n_fatent)  /* Check range */
    ...
    switch (fs->fs_type) {
    case FS_FAT12 : {
      UINT bc = (UINT)clst; bc += bc / 2;
      if (moveWindow (fs, fs->fatbase + (bc / _MAX_SS)) != FR_OK)
        break;

      UINT wc = fs->win.d8[bc++ % _MAX_SS];
      if (moveWindow (fs, fs->fatbase + (bc / _MAX_SS)) != FR_OK)
        break;
    ...
    case FS_FAT16 : {
      if (moveWindow(fs, fs->fatbase + (clst / (_MAX_SS / 2))) != FR_OK)
        break;
      BYTE* p = &fs->win.d8[clst * 2 % _MAX_SS];
      val = LD_WORD(p);
      break;
      }
    ...
    case FS_FAT32 : {
      if (moveWindow(fs, fs->fatbase + (clst / (_MAX_SS / 4))) != FR_OK)
        break;
      BYTE* p = &fs->win.d8[clst * 4 % _MAX_SS];
...
static DWORD create_chain (FATFS* fs, DWORD clst) {
  ...
  if (clst == 0) {    /* Create a new chain */
    ...
    DWORD cs = get_fat(fs, clst);     /* Check the cluster status */
    if (cs < 2)
 ...
 DWORD ncl = scl;        /* Start cluster */
  for (;;) {
    ...
    DWORD cs = get_fat(fs, ncl);      /* Get the cluster status */
    if (cs == 0)
  ...
  FRESULT res = put_fat(fs, ncl, 0x0FFFFFFF); /* Mark the new cluster "last link" */
  if (res == FR_OK && clst != 0)
  ...
  if (!(i % (_MAX_SS / SZ_DIRE))) {
    ...
    if (!dp->clust) {
      ...
      if (((i / (_MAX_SS / SZ_DIRE)) & (dp->fs->csize - 1)) == 0) {
        /* Cluster changed? */
        DWORD clst = get_fat (dp->fs, dp->clust);        /* Get next cluster */
        if (clst <= 1)
          return FR_INT_ERR;
        if (clst == 0xFFFFFFFF)
          return FR_DISK_ERR;
...
static FRESULT dir_read (DIR* dp, int vol) {
  ...
  while (dp->sect) {
    ...
    BYTE* dir = dp->dir;          /* Ptr to the directory entry of current index */
    BYTE c = dir[DIR_Name];
    if (c == 0) {
      res = FR_NO_FILE;
      break;
...
static FRESULT dir_remove (DIR* dp) {
  ...
  if (res == FR_OK) {
    do {
      ...
      /* Clear and mark the entry "deleted" */
      memset (dp->dir, 0, SZ_DIRE);
...
static FRESULT find_volume (FATFS** rfs, const TCHAR** path, BYTE wmode) {
  ...
  *rfs = fs;
  ...
  /* file system object is not valid, attempt to mount the volume. (analyze BPB and initialize the fs object) */
  ...
  DWORD bsect = 0;
  BYTE fmt = checkFs (fs, bsect);          /* Load sector 0 and check if it is an FAT boot sector as SFD */
  if (fmt == 1 || (!fmt && vol)) {
  ...
  DWORD fasize = LD_WORD(fs->win.d8 + BPB_FATSz16);
  if (!fasize)
    fasize = LD_DWORD(fs->win.d8 + BPB_FATSz32);
  fs->fsize = fasize;

  /* Number of FAT copies */
  fs->n_fats = fs->win.d8[BPB_NumFATs];
  if (fs->n_fats != 1 && fs->n_fats != 2)       /* (Must be 1 or 2) */
    return FR_NO_FILESYSTEM;
  fasize *= fs->n_fats;               /* Number of sectors for FAT area */

  /* Number of sectors per cluster */
  fs->csize = fs->win.d8[BPB_SecPerClus];
  if (!fs->csize || (fs->csize & (fs->csize - 1)))  /* (Must be power of 2) */
    return FR_NO_FILESYSTEM;

  /* Number of root directory entries */
  fs->n_rootdir = LD_WORD(fs->win.d8 + BPB_RootEntCnt);
  if (fs->n_rootdir % (_MAX_SS / SZ_DIRE))       /* (Must be sector aligned) */
    return FR_NO_FILESYSTEM;

  /* Number of sectors on the volume */
  DWORD tsect = LD_WORD(fs->win.d8 + BPB_TotSec16);
  if (!tsect)
    tsect = LD_DWORD(fs->win.d8 + BPB_TotSec32);

  /* Number of reserved sectors */
  WORD nrsv = LD_WORD(fs->win.d8 + BPB_RsvdSecCnt);
  if (!nrsv)
    return FR_NO_FILESYSTEM;         /* (Must not be 0) */

  /* Determine the FAT sub type */
  DWORD sysect = nrsv + fasize + fs->n_rootdir / (_MAX_SS / SZ_DIRE);  /* RSV + FAT + DIR */
  if (tsect < sysect)
    return FR_NO_FILESYSTEM;    /* (Invalid volume size) */

  /* Number of clusters */
  DWORD nclst = (tsect - sysect) / fs->csize;
  if (!nclst)
    return FR_NO_FILESYSTEM;        /* (Invalid volume size) */
  fmt = FS_FAT12;
  if (nclst >= MIN_FAT16)
...
static FRESULT follow_path (DIR* dp, const TCHAR* path) {
  ...
  if ((UINT)*path < ' ') {
    ...
    for (;;) {
      ...
      /* Find an object with the sagment name */
      res = dir_find (dp);
      BYTE ns = dp->fn[NSFLAG];
      if (res != FR_OK) {
        /* Failed to find the object */
        if (res == FR_NO_FILE) {
          /* Object is not found */
          if (ns & NS_DOT) {
            /* If dot entry is not exist,  it is the root directory and stay there */
            dp->sclust = 0;
            dp->dir = 0;
            if (!(ns & NS_LAST))
    ...
    if (fs->free_clust <= fs->n_fatent - 2)
      *nclst = fs->free_clust;
      ...
      if (fat == FS_FAT12) {
        ...
        DWORD clst = fs->n_fatent;
        DWORD sect = fs->fatbase;
        UINT i = 0;
        BYTE* p = 0;
        do {
          if (!i) {
...
FRESULT f_open (FIL* fp, const TCHAR* path, BYTE mode) {
  ...
  if (res == FR_OK) {
    WCHAR* lfn = pvPortMalloc ((_MAX_LFN + 1) * 2);
    dj.lfn = lfn;
    dj.fn = sfn;
    ...
    if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
      ...
      if (res == FR_OK && (mode & FA_CREATE_ALWAYS)) {
        ...
        DWORD dw = GET_FATTIME();       /* Created time */
        ST_DWORD(dir + DIR_CrtTime, dw);
        dir[DIR_Attr] = 0;        /* Reset attribute */
        ST_DWORD(dir + DIR_FileSize, 0);/* size = 0 */
        DWORD cl = loadCluster (dj.fs, dir);    /* Get start cluster */
    ...
    if (res == FR_OK) {
      ...
      fp->sclust = loadCluster (dj.fs, dir);       /* File start cluster */
...
FRESULT f_read (FIL* fp, void* buff, UINT btr, UINT* br) {
  ...
  *br = 0;  /* Clear read byte counter */
  ...
  DWORD remain = fp->fsize - fp->fptr;
  if (btr > remain)
    btr = (UINT)remain;   /* Truncate btr by remaining bytes */

  BYTE* rbuff = (BYTE*)buff;
  UINT rcnt;
  for ( ;  btr; /* Repeat until all data read */ rbuff += rcnt, fp->fptr += rcnt, *br += rcnt, btr -= rcnt) {
    if ((fp->fptr % _MAX_SS) == 0) {
      /* On the sector boundary? */
      BYTE csect = (BYTE)(fp->fptr / _MAX_SS & (fp->fs->csize - 1));  /* Sector offset in the cluster */
      DWORD clst;
      if (!csect) {
      ...
      DWORD sect = clusterToSector (fp->fs, fp->clust); /* Get current sector */
      if (!sect)
        ABORT(fp->fs, FR_INT_ERR);
      sect += csect;
      UINT cc = btr / _MAX_SS; /* When remaining bytes >= sector size, */
      if (cc) {
        ...
        if ((fp->flag & FA__DIRTY) && fp->dsect - sect < cc)
          memcpy(rbuff + ((fp->dsect - sect) * _MAX_SS), fp->buf.d8, _MAX_SS);
    ...
    rcnt = _MAX_SS - ((UINT)fp->fptr % _MAX_SS);  /* Get partial sector data from sector buffer */
    if (rcnt > btr)
      rcnt = btr;
    memcpy (rbuff, &fp->buf.d8[fp->fptr % _MAX_SS], rcnt); /* Pick partial sector */
    }
...
FRESULT f_write (FIL* fp, const void *buff, UINT btw, UINT* bw) {
  ...
  *bw = 0;  /* Clear write byte counter */
  ...
  for ( ;  btw;             /* Repeat until all data written */
    ...
    if ((fp->fptr % _MAX_SS) == 0) { /* On the sector boundary? */
      csect = (BYTE)(fp->fptr / _MAX_SS & (fp->fs->csize - 1));  /* Sector offset in the cluster */
      if (!csect) {
        /* On the cluster boundary? */
        if (fp->fptr == 0) {
          /* On the top of the file? */
      ...
      sect = clusterToSector(fp->fs, fp->clust); /* Get current sector */
      if (!sect)
        ABORT(fp->fs, FR_INT_ERR);
      sect += csect;
      ...
      if (cc) {           /* Write maximum contiguous sectors directly */
        ...
        if (fp->dsect - sect < cc) { /* Refill sector cache if it gets invalidated by the direct write */
          memcpy(fp->buf.d8, wbuff + ((fp->dsect - sect) * _MAX_SS), _MAX_SS);
          fp->flag &= ~FA__DIRTY;
          }
    ...
    wcnt = _MAX_SS - ((UINT)fp->fptr % _MAX_SS); /* Put partial sector into file I/O buffer */
    if (wcnt > btw)
      wcnt = btw;
    memcpy (&fp->buf.d8[fp->fptr % _MAX_SS], wbuff, wcnt); /* Fit partial sector */
    fp->flag |= FA__DIRTY;
    }
...
FRESULT f_sync (FIL* fp) {
  ...
  if (res == FR_OK) {
    if (fp->flag & FA__WRITTEN) {
      ...
      res = moveWindow (fp->fs, fp->dir_sect);
      if (res == FR_OK) {
        BYTE* dir = fp->dir_ptr;
        dir[DIR_Attr] |= AM_ARC;          /* Set archive bit */
        ST_DWORD(dir + DIR_FileSize, fp->fsize);  /* Update file size */
        storeCluster (dir, fp->sclust);          /* Update start cluster */

        DWORD tm = GET_FATTIME();             /* Update updated time */
...
FRESULT f_lseek (FIL* fp, DWORD ofs) {
  ...
  if (fp->cltbl) {
    ...
    if (ofs == CREATE_LINKMAP) {
      ...
      *fp->cltbl = ulen;
      ...
      if (ofs) {
        ...
        dsc = clusterToSector (fp->fs, fp->clust);
        if (!dsc) ABORT(fp->fs, FR_INT_ERR);
        dsc += (ofs - 1) / _MAX_SS & (fp->fs->csize - 1);
        if (fp->fptr % _MAX_SS && dsc != fp->dsect) {  /* Refill sector cache if needed */
    ...
    fp->fptr = nsect = 0;
    if (ofs) {
      bcs = (DWORD)fp->fs->csize * _MAX_SS;  /* Cluster size (byte) */
      ...
      if (clst != 0) {
        ...
        fp->fptr += ofs;
        if (ofs % _MAX_SS) {
          nsect = clusterToSector (fp->fs, clst); /* Current sector */
          if (!nsect)
            ABORT(fp->fs, FR_INT_ERR);
          nsect += ofs / _MAX_SS;
          }
    ...
    if (fp->fptr % _MAX_SS && nsect != fp->dsect) {
      ...
      if (disk_read (fp->fs->drv, fp->buf.d8, nsect, 1) != RES_OK)
        /* Fill sector cache */
        ABORT(fp->fs, FR_DISK_ERR);
      fp->dsect = nsect;
      }
    ...
    if (res == FR_OK) {
      ...
      if (dp->dir) {
        ...
        if (dp->dir[DIR_Attr] & AM_DIR)
          ...
          dp->sclust = loadCluster (fs, dp->dir);
        else
  ...
  if (res == FR_OK) {
    ...
    while ((ccl = dj.sclust) != 0) {  /* Repeat while current directory is a sub-directory */
      ...
      do {              /* Find the entry links to the child directory */
        res = dir_read(&dj, 0);
        if (res != FR_OK) break;
        if (ccl == loadCluster (dj.fs, dj.dir)) break;  /* Found the entry */
        res = dir_next(&dj, 0);
      } while (res == FR_OK);
