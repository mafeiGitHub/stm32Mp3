...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef *heth) {
  ...
  /* Ethernet Software reset Set the SWR bit: resets all MAC subsystem internal registers and logic */
  ...
  while (((heth->Instance)->DMABMR & ETH_DMABMR_SR) != (uint32_t)RESET) {
    if ((HAL_GetTick() - tickstart ) > ETH_TIMEOUT_SWRESET) {
      ...
      __HAL_UNLOCK(heth);
      /* SWR is not performed if ETH_RX_CLK or ETH_TX_CLK are not available */
      return HAL_TIMEOUT;
    }
  }
