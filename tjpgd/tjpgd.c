// TJpgD.h - Tiny JPEG Decompressor R0.01b (C)ChaN, 2012
#include "tjpgd.h"
...
// Create de-quantization and prescaling tables with a DQT segment       */
  ...
  while (ndata) {
    ...
    jd->qttbl[i] = pb;
    for (i = 0; i < 64; i++) {
      // Load the table, apply scale factor of Arai algorithm to the de-quantizers
      z = Zig[i];
...
// Create huffman code tables with a DHT segment
  ...
  while (ndata) {
    ...
    BYTE d = *data++;      // Get table number and class
    UINT cls = (d >> 4);
    UINT num = d & 0x0F;   // class = dc(0)/ac(1), table number = 0/1
    if (d & 0xEE)
      return JDR_FMT1;

    // Allocate a memory block for the bit distribution table
    BYTE* pb = alloc_pool (jd, 16);
    ...
    WORD* ph = alloc_pool (jd, np * sizeof (WORD));
...
// Extract a huffman decoded data from input stream                      */
  ...
  BYTE msk = jd->dmsk;
  UINT dc = jd->dctr;
  BYTE* dp = jd->dptr; /* Bit mask, number of data available, read ptr */
  BYTE s = *dp;
  UINT v = 0;
  UINT f = 0;
  UINT bl = 16;  /* Max code length */
  do {
    if (!msk) {   /* Next byte? */
      if (!dc) {  /* No input data is available, re-fill input buffer */
        dp = jd->inbuf; /* Top of input buffer */
...
// Apply Inverse-DCT in Arai Algorithm (see also aa_idct.png)
  ...
  const LONG M13 = (LONG)(1.41421*4096);
  const LONG M2 = (LONG)(1.08239*4096);
  const LONG M4 = (LONG)(2.61313*4096);
  const LONG M5 = (LONG)(1.84776*4096);
...
static JRESULT mcu_load (JDEC* jd) {
  ...
  UINT nby = jd->msx * jd->msy; // Number of Y blocks (1, 2 or 4)
  UINT nbc = 2;                 // Number of C blocks (2)
  BYTE* bp = jd->mcubuf;        // Pointer to the first block
  UINT blk;
  for (blk = 0; blk < nby + nbc; blk++) {
    ...
    do {
      ...
      if (b == 0)
        break;              // EOB? */
      if (b < 0)
        return 0 - b;       // Err: invalid code or input error */
...
//}}}
//{{{
static JRESULT mcu_output (JDEC* jd, UINT (*outfunc)(JDEC*, BYTE*, JRECT*), UINT x, UINT y) {
// Output an MCU: Convert YCrCb to RGB and output in RGB
  ...
  UINT mx = jd->msx * 8;
  UINT my = jd->msy * 8;
  UINT rx = ((x + mx <= jd->width) ? mx : jd->width - x) >> jd->scale;
  UINT ry = ((y + my <= jd->height) ? my : jd->height - y) >> jd->scale;
  if (!rx || !ry)
  ...
  if (jd->scale == 3) {
    ...
    for (iy = 0; iy < my; iy++) {
      ...
      for (ix = 0; ix < mx; ix++) {
        ...
        if (mx == 16) {
          ...
          // Single block width, Increase chroma pointer every pixel
          pc++;
    ...
    if (jd->scale) {
      //{{{  Descale the MCU rectangule
      UINT x, y, v1, v2, v3, s, w, a;
      BYTE *op;

      // Get averaged value of each square correcponds to a pixel
...
//}}}
//{{{
JRESULT jd_decomp (JDEC* jd, UINT (*outfunc)(JDEC*, BYTE*, JRECT*), BYTE scale) {

  jd->scale = (scale > 3) ? 3 : scale;
